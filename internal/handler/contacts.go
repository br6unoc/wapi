...
func APIListContacts(c *gin.Context) {
	companyID := currentCompanyID(c)
	rows, err := postgres.DB.Query(`
		SELECT c.id, c.phone, COALESCE(c.name,''), c.purchase_count, c.is_blocked,
		       COALESCE(c.conv_status,''), TO_CHAR(c.last_contact_at, 'YYYY-MM-DD"T"HH24:MI:SS"Z"')
		FROM contacts c
		JOIN instances i ON i.id = c.instance_id
		WHERE i.company_id = $1
		ORDER BY c.last_contact_at DESC NULLS LAST
	...
	for rows.Next() {
		var r row
		var lastAt *string
		rows.Scan(&r.ID, &r.Phone, &r.Name, &r.PurchaseCount, &r.IsBlocked, &r.ConvStatus, &lastAt)
		r.LastContactAt = lastAt
